...
struct block_tq2_0x8 {
    ggml_half d[8];              // deltas for 8 tq2_0 blocks
    uint8_t   qs[QK_K / 4 * 8];  // 2-bit quants for 8 tq2_0 blocks, byte-interleaved
};

static_assert(sizeof(block_tq2_0x8) == 8 * sizeof(ggml_half) + QK_K / 4 * 8, "wrong tq2_0x8 block size/padding");
static_assert(sizeof(block_tq2_0x8) == 8 * sizeof(block_tq2_0), "block_tq2_0x8 must cover 8 packed block_tq2_0 rows exactly");
...
// The intrinsics twin of ggml_gemm_q4_K_8x8_q8_K, kept compiled on aarch64 so that
// tests/test-repack-q4_K-head.cpp can hold the inline asm kernel to it bit for bit.
...
// 2-row TQ2_0 GEMM (i8mm). vy is a block_q8_Kx4 activation panel of which only the first
// row pair is read, and nr must be 2. See ggml_gemm2_tq2_0_8x8_q8_K in arch/arm/repack.cpp.
void ggml_gemm2_tq2_0_8x8_q8_K(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc);
