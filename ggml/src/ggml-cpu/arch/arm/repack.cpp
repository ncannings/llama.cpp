...
// ---------------------------------------------------------------------------------------
// q4_K 8x8 i8mm GEMM, EIGHT activation rows per pass.
//
// The 4-row kernel above re-reads a superblock's weight nibbles, scales and mins for four
// rows of reuse: each unpacked weight vector feeds two SMMLA. Taking two q8_K 4-row panels
// per pass makes it feed four. A full 8x8 tile would want 16 acc + 16 chain + 16 weight +
// 16 activation vectors, four times the register file, so exactly one of weights or
// activations has to be read twice per plane. A weight vector costs a load AND a mask, an
// activation vector costs a load, so the activations are the ones re-read: the superblock
// is walked twice, once per COLUMN HALF (column pairs 0,1 then 2,3), and inside a pass the
// two row halves share one set of unpacked weights.
//
//   v0-v7   acc: (column pair a,b) x (row pair 0..3)   v24-v27  the four SMMLA chains
//   v8-v15  activations, one row half                  v28      0x0f mask
//   v16-v19 4-bit weights, column pair a               v30-v31  the two int32 scale vectors
//   v20-v23 4-bit weights, column pair b               v29      free
//
// Per superblock that leaves the weight loads and masks where the 4-row kernel had them
// (128 + 128) while doubling the SMMLA to 512, doubles the activation loads (128 to 256),
// and halves the per-row cost of the two fixed items: the 512 byte scale table is built
// once for eight rows instead of once for four, and the mins pass decodes each 6-bit min
// set once and applies it to both panels. The column half is expressed by pre-offsetting
// the pointers (weights +32 bytes, scale table +32 bytes, output +16 bytes), so one asm
// body serves both passes.
//
// The mins pass keeps its sixteen accumulators in v16-v31 and the two bsums vectors in
// v0-v1 because SMLAL/SMLAL2 by element require the indexed operand in V0-V15.
//
// Identity. Every chain starts at zero and takes k groups 0,1,2,3 in order; each acc takes
// the low nibble plane of sub-block 2t scaled by that sub-block's scale before the high
// nibble plane of sub-block 2t+1, for t = 0..3, which is the 4-row order and therefore the
// intrinsics order. The mins pass accumulates sub-block A then sub-block B within each
// group and the groups in order, the same widening and the same lane pattern. Rows are
// independent in the float fold and each row's fold is the 4-row kernel's, unchanged, so
// the result is bit identical per row and not merely close.
// ---------------------------------------------------------------------------------------

#define GGML_Q4K88R8_W(MN, ARG)                              \
    "ldr   q16, [x9, #0]\n"                                  \
    "ldr   q17, [x9, #64]\n"                                 \
    "ldr   q18, [x9, #128]\n"                                \
    "ldr   q19, [x9, #192]\n"                                \
...
#define GGML_Q4K88R8_SC(SCOFF)                               \
    "ldr   q30, [x10, #(" SCOFF ")]\n"                       \
    "ldr   q31, [x10, #(" SCOFF " + 16)]\n"

#define GGML_Q4K88R8_Q8(REG, BASE)                           \
    "ldr   q8,  [" REG ", #(" BASE ")]\n"                    \
    "ldr   q9,  [" REG ", #(" BASE " + 32)]\n"               \
    "ldr   q10, [" REG ", #(" BASE " + 64)]\n"               \
    "ldr   q11, [" REG ", #(" BASE " + 96)]\n"               \
    ...
    "ldr   q13, [" REG ", #(" BASE " + 48)]\n"               \
    "ldr   q14, [" REG ", #(" BASE " + 80)]\n"               \
    "ldr   q15, [" REG ", #(" BASE " + 112)]\n"

#define GGML_Q4K88R8_CHAIN(A0, A1, B0, B1)                   \
    "movi  v24.4s, #0\n"                                     \
    "movi  v25.4s, #0\n"                                     \
    "movi  v26.4s, #0\n"                                     \
    "movi  v27.4s, #0\n"                                     \
    ...
    "smmla v26.4s, v23.16b, v11.16b\n"                       \
    "smmla v27.4s, v23.16b, v15.16b\n"                       \
    "mla   " A0 ".4s, v24.4s, v30.4s\n"                      \
    "mla   " A1 ".4s, v25.4s, v30.4s\n"                      \
    "mla   " B0 ".4s, v26.4s, v31.4s\n"                      \
    "mla   " B1 ".4s, v27.4s, v31.4s\n"

#define GGML_Q4K88R8_PLANE(MN, ARG, SCOFF, Q8BASE)           \
    GGML_Q4K88R8_W(MN, ARG)                                  \
    GGML_Q4K88R8_SC(SCOFF)                                   \
    GGML_Q4K88R8_Q8("x15", Q8BASE)                           \
    GGML_Q4K88R8_CHAIN("v0", "v2", "v1", "v3")               \
    GGML_Q4K88R8_Q8("x16", Q8BASE)                           \
    GGML_Q4K88R8_CHAIN("v4", "v6", "v5", "v7")

// acc v0,v1 = (cp a, cp b) x row pair 0 ... v6,v7 = row pair 3. SMMLA leaves
// {c0r0,c0r1,c1r0,c1r1} in each, so UZP1 of a column pair is the even row of the pair and
// UZP2 the odd row, four columns of it; rows are 32 bytes apart in the output block.
#define GGML_Q4K88R8_EPI                                     \
    "uzp1  v16.4s, v0.4s, v1.4s\n"                           \
...
// one column half of the quants pass: four columns x eight rows over all eight sub-blocks.
// out points at the first int32 of those four columns in row 0 of the 8x8 output block.
static inline void ggml_q4_K_8x8x8_quants(const uint8_t * GGML_RESTRICT q4_qs,
                                          const int32_t * GGML_RESTRICT tbl,
                                          const int8_t * GGML_RESTRICT  q8a,
                                          const int8_t * GGML_RESTRICT  q8b,
                                          int32_t * GGML_RESTRICT       out) {
    __asm__ volatile(
        "mov   x9,  %[q4]\n"
        "mov   x10, %[tbl]\n"
        ...
        "movi  v4.4s, #0\n" "movi  v5.4s, #0\n" "movi  v6.4s, #0\n" "movi  v7.4s, #0\n"
        "1:\n"
        GGML_Q4K88R8_PLANE("and ", "v28.16b", "0",  "0")
        GGML_Q4K88R8_PLANE("ushr", "#4",      "64", "128")
        "add   x9,  x9,  #256\n"
        "add   x10, x10, #128\n"
        "add   x15, x15, #256\n"
        "add   x16, x16, #256\n"
        ...
        GGML_Q4K88R8_EPI
        :
        : [q4] "r"(q4_qs), [tbl] "r"(tbl), [q8a] "r"(q8a), [q8b] "r"(q8b), [out] "r"(out)
        : "memory", "cc", "x8", "x9", "x10", "x15", "x16",
          "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12",
          "v13", "v14", "v15", "v16", "v17", "v18", "v19", "v20", "v21", "v22", "v23",
          "v24", "v25", "v26", "v27", "v28", "v29", "v30", "v31");
}

// decode_q_Kx8_6bit_scales, mins half only; same masks, same shifts, same vmovl_u8 widening
#define GGML_Q4K88R8_MINDEC(DN, VN, SCOFF)                   \
    "ldr   w11, [%[sc], #(" SCOFF " + 4)]\n"                 \
    "ldr   w13, [%[sc], #(" SCOFF " + 8)]\n"                 \
    "and   w12, w11, %w[km1]\n"                              \
    "lsr   w14, w11, #6\n"                                   \
...
#define GGML_Q4K88R8_MINROW(L, H, BS, I0, I1)                \
    "smlal  " L ".4s, v2.4h, " BS ".h[" I0 "]\n"             \
    "smlal  " L ".4s, v3.4h, " BS ".h[" I1 "]\n"             \
    "smlal2 " H ".4s, v2.8h, " BS ".h[" I0 "]\n"             \
    "smlal2 " H ".4s, v3.8h, " BS ".h[" I1 "]\n"

#define GGML_Q4K88R8_MINS(BSOFF, SCOFF)                      \
    "ldr   q4, [%[bsa], #(" BSOFF ")]\n"                     \
    "ldr   q5, [%[bsa], #(" BSOFF " + 16)]\n"                \
    "addp  v0.8h, v4.8h, v5.8h\n"                            \
    "ldr   q4, [%[bsb], #(" BSOFF ")]\n"                     \
    "ldr   q5, [%[bsb], #(" BSOFF " + 16)]\n"                \
    "addp  v1.8h, v4.8h, v5.8h\n"                            \
    GGML_Q4K88R8_MINDEC("d2", "v2", SCOFF)                   \
    GGML_Q4K88R8_MINDEC("d3", "v3", SCOFF " + 12")           \
    GGML_Q4K88R8_MINROW("v16", "v17", "v0", "0", "1")        \
    GGML_Q4K88R8_MINROW("v18", "v19", "v0", "2", "3")        \
    GGML_Q4K88R8_MINROW("v20", "v21", "v0", "4", "5")        \
    GGML_Q4K88R8_MINROW("v22", "v23", "v0", "6", "7")        \
    GGML_Q4K88R8_MINROW("v24", "v25", "v1", "0", "1")        \
    GGML_Q4K88R8_MINROW("v26", "v27", "v1", "2", "3")        \
    GGML_Q4K88R8_MINROW("v28", "v29", "v1", "4", "5")        \
    GGML_Q4K88R8_MINROW("v30", "v31", "v1", "6", "7")

// the mins pass for both panels at once; out points at the 64 int32 mins area
static inline void ggml_q4_K_8x8x8_mins(const uint8_t * GGML_RESTRICT q4_sc,
                                        const int16_t * GGML_RESTRICT bsa,
                                        const int16_t * GGML_RESTRICT bsb,
                                        int32_t * GGML_RESTRICT       out) {
    __asm__ volatile(
        "movi  v16.4s, #0\n" "movi  v17.4s, #0\n" "movi  v18.4s, #0\n" "movi  v19.4s, #0\n"
        "movi  v20.4s, #0\n" "movi  v21.4s, #0\n" "movi  v22.4s, #0\n" "movi  v23.4s, #0\n"
        "movi  v24.4s, #0\n" "movi  v25.4s, #0\n" "movi  v26.4s, #0\n" "movi  v27.4s, #0\n"
        "movi  v28.4s, #0\n" "movi  v29.4s, #0\n" "movi  v30.4s, #0\n" "movi  v31.4s, #0\n"
        GGML_Q4K88R8_MINS("0",  "0")
        GGML_Q4K88R8_MINS("32", "24")
        GGML_Q4K88R8_MINS("64", "48")
        GGML_Q4K88R8_MINS("96", "72")
        "stp   q16, q17, [%[out], #0]\n"
        "stp   q18, q19, [%[out], #32]\n"
        "stp   q20, q21, [%[out], #64]\n"
        "stp   q22, q23, [%[out], #96]\n"
        ...
        "stp   q30, q31, [%[out], #224]\n"
        :
        : [sc] "r"(q4_sc), [bsa] "r"(bsa), [bsb] "r"(bsb), [out] "r"(out),
          [km1] "r"(0x3f3f3f3fu), [km2] "r"(0x0f0f0f0fu), [km3] "r"(0x03030303u)
        : "memory", "cc", "x11", "x12", "x13", "x14",
          "v0", "v1", "v2", "v3", "v4", "v5",
          "v16", "v17", "v18", "v19", "v20", "v21", "v22", "v23",
          "v24", "v25", "v26", "v27", "v28", "v29", "v30", "v31");
}

// the 512 byte sub-block scale table, built once for eight rows
static inline void ggml_q4_K_8x8x8_table(const uint8_t * GGML_RESTRICT q4_sc,
                                         int32_t * GGML_RESTRICT       tbl) {
    __asm__ volatile(
        GGML_Q4K88_DECODE("0",  "0")
        GGML_Q4K88_DECODE("12", "64")
        ...
        GGML_Q4K88_DECODE("84", "448")
        :
        : [sc] "r"(q4_sc), [tbl] "r"(tbl),
          [km1] "r"(0x3f3f3f3fu), [km2] "r"(0x0f0f0f0fu), [km3] "r"(0x03030303u)
        : "memory", "cc", "x11", "x12", "x13", "x14",
          "v24", "v25", "v26", "v27", "v28", "v29", "v31");
}

// panels [y0, y1) taken two at a time, eight rows per pass
static void ggml_gemm_q4_K_8x8_q8_K_range8(int                        n,
                                           float * GGML_RESTRICT      s,
                                           size_t                     bs,
                                           const void * GGML_RESTRICT vx,
                                           const void * GGML_RESTRICT vy,
    ...
    float32x4_t         acc_f32[16];
    alignas(16) int32_t accbuf[128];
    alignas(16) int32_t sctbl[128];

    for (int y = y0; y + 1 < y1; y += 2) {
        const block_q8_Kx4 * GGML_RESTRICT q8a = (const block_q8_Kx4 *) vy + (y * nb);
        const block_q8_Kx4 * GGML_RESTRICT q8b = (const block_q8_Kx4 *) vy + ((y + 1) * nb);
        ...
        for (int x = 0; x < nc / ncols_interleaved; x++) {
            ...
            for (int b = 0; b < nb; b++) {
                ggml_q4_K_8x8x8_table(q4_ptr[b].scales, sctbl);
                ggml_q4_K_8x8x8_quants(q4_ptr[b].qs, sctbl, q8a[b].qs, q8b[b].qs, accbuf);
                ggml_q4_K_8x8x8_quants(q4_ptr[b].qs + 32, sctbl + 8, q8a[b].qs, q8b[b].qs, accbuf + 4);
                ggml_q4_K_8x8x8_mins(q4_ptr[b].scales, q8a[b].bsums, q8b[b].bsums, accbuf + 64);
                ...
                for (int i = 0; i < 2 * q8_k_blocklen; i++) {
                    const float32x4_t q8_d = vdupq_n_f32(i < q8_k_blocklen ? q8a[b].d[i] : q8b[b].d[i - q8_k_blocklen]);

                    const float32x4_t dmins0 = vmulq_f32(q4_dmin_0, q8_d);
                    const float32x4_t scale0 = vmulq_f32(q4_d_0, q8_d);
                    acc_f32[2 * i] = vmlsq_f32(acc_f32[2 * i], vcvtq_f32_s32(vld1q_s32(accbuf + 64 + 8 * i)), dmins0);
                    ...
                    acc_f32[2 * i + 1] = vmlsq_f32(acc_f32[2 * i + 1], vcvtq_f32_s32(vld1q_s32(accbuf + 64 + 8 * i + 4)), dmins1);
...
// panels [y0, y1), four rows per pass. This is the kernel the 8-row tile was measured
// against and the one a trailing odd panel still runs.
static void ggml_gemm_q4_K_8x8_q8_K_range4(int                        n,
                                           float * GGML_RESTRICT      s,
                                           size_t                     bs,
                                           const void * GGML_RESTRICT vx,
                                           const void * GGML_RESTRICT vy,
...
// The exported 4-row kernel. tests/test-repack-q4_K-head.cpp holds the dispatched entry
// point to this as well as to the intrinsics kernel, so that the 8-row tile is checked
// against the code it actually replaces and not only against the original reference.
void ggml_gemm_q4_K_8x8_q8_K_asm4(int                        n,
                                  float * GGML_RESTRICT      s,
                                  size_t                     bs,
                                  const void * GGML_RESTRICT vx,
                                  const void * GGML_RESTRICT vy,
    ...
    assert(nr % 4 == 0);
    assert(nc % 8 == 0);
    ggml_gemm_q4_K_8x8_q8_K_range4(n, s, bs, vx, vy, 0, nr / 4, nc);
#else
    ggml_gemm_q4_K_8x8_q8_K_intrinsics(n, s, bs, vx, vy, nr, nc);
#endif  // GGML_Q4_K_8X8_GEMM_ASM
}

// The dispatched entry point. forward_mul_mat chunks src1 by whole planes, so nr is the
// batch rounded down to a multiple of 4 and this sees eight rows exactly when ne11 >= 8.
// Whole groups of eight go through the 8-row tile; a trailing odd panel keeps the 4-row
// kernel, which is why the identity test covers every residue of 8 as well as of 4.
void ggml_gemm_q4_K_8x8_q8_K(int                        n,
    ...
    const int npanel = nr / 4;
    const int paired = (npanel / 2) * 2;

    if (paired > 0) {
        ggml_gemm_q4_K_8x8_q8_K_range8(n, s, bs, vx, vy, 0, paired, nc);
    }
    if (paired < npanel) {
        ggml_gemm_q4_K_8x8_q8_K_range4(n, s, bs, vx, vy, paired, npanel, nc);
    }
    return;
#else
    ggml_gemm_q4_K_8x8_q8_K_intrinsics(n, s, bs, vx, vy, nr, nc);
...
// TQ2_0 repacked kernels. Weights are raw 2-bit codes {0,1,2} byte-interleaved over 8 columns
// (see block_tq2_0x8); source byte b of a row at shift 2*sh is element (b / 32) * 128 + sh * 32 + b % 32.
// The codes are dotted unsigned and the "-1" is folded into one bsums total per block and token,
// exactly as ggml_vec_dot_tq2_0_q8_K does, so the results match the single-row path.
#if defined(__aarch64__) && defined(__ARM_NEON) && defined(__ARM_FEATURE_DOTPROD)
static inline int8x16_t tq2_0_codes_0(uint8x16_t v, uint8x16_t m3) { return vreinterpretq_s8_u8(vandq_u8(v, m3)); }
static inline int8x16_t tq2_0_codes_2(uint8x16_t v, uint8x16_t m3) { return vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(v, 2), m3)); }
static inline int8x16_t tq2_0_codes_4(uint8x16_t v, uint8x16_t m3) { return vreinterpretq_s8_u8(vandq_u8(vshrq_n_u8(v, 4), m3)); }
static inline int8x16_t tq2_0_codes_6(uint8x16_t v) { return vreinterpretq_s8_u8(vshrq_n_u8(v, 6)); }

// Sum of the 16 bsums of a block_q8_K: the correction for the -1 offset of every code
static inline int32x4_t tq2_0_q8_K_bsum(const block_q8_K * q8) {
    const int16x8_t ysum0 = vld1q_s16(q8->bsums);
    const int16x8_t ysum1 = vld1q_s16(q8->bsums + 8);
    return vdupq_n_s32(vaddvq_s32(vpaddlq_s16(vaddq_s16(ysum0, ysum1))));
}

// Per-token bsums totals of a block_q8_Kx4, lanes = tokens 0..3.
// bsums are stored as [group of 4 sub-blocks][token][4], see ggml_quantize_mat_q8_K_4x4/4x8
static inline int32x4_t tq2_0_q8_Kx4_bsum(const block_q8_Kx4 * q8) {
    int16x8_t lo = vld1q_s16(q8->bsums);       // t0 x4, t1 x4
    int16x8_t hi = vld1q_s16(q8->bsums + 8);   // t2 x4, t3 x4
    for (int g = 1; g < 4; g++) {
        lo = vaddq_s16(lo, vld1q_s16(q8->bsums + 16 * g));
        hi = vaddq_s16(hi, vld1q_s16(q8->bsums + 16 * g + 8));
    }
...
void ggml_gemv_tq2_0_8x4_q8_K(int                        n,
    ...
    for (int x = 0; x < nc / ncols_interleaved; x++) {
        ...
        for (int b = 0; b < nb; b++) {
            // acc[c] holds the raw code dot products of columns 4c..4c+3
            int32x4_t acc[2] = { vdupq_n_s32(0), vdupq_n_s32(0) };

            for (int h = 0; h < 2; h++) {
                // q8[sh][q] covers elements h*128 + sh*32 + q*16 .. +15
                ...
                // chunk k = 32 bytes: 8 columns x source bytes h*32 + k*4 .. +3
                ...
                for (int q = 0; q < 2; q++) {
                    for (int c = 0; c < 2; c++) {
                        ...
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_0(w0, m3), q8[0][q], 0);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_0(w1, m3), q8[0][q], 1);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_0(w2, m3), q8[0][q], 2);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_0(w3, m3), q8[0][q], 3);

                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_2(w0, m3), q8[1][q], 0);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_2(w1, m3), q8[1][q], 1);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_2(w2, m3), q8[1][q], 2);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_2(w3, m3), q8[1][q], 3);

                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_4(w0, m3), q8[2][q], 0);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_4(w1, m3), q8[2][q], 1);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_4(w2, m3), q8[2][q], 2);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_4(w3, m3), q8[2][q], 3);

                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_6(w0), q8[3][q], 0);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_6(w1), q8[3][q], 1);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_6(w2), q8[3][q], 2);
                        acc[c] = vdotq_laneq_s32(acc[c], tq2_0_codes_6(w3), q8[3][q], 3);
                    }
                }
            }
            ...
            for (int c = 0; c < 2; c++) {
                ...
                acc_f32[c] = vfmaq_f32(acc_f32[c], scale, vcvtq_f32_s32(vsubq_s32(acc[c], bsum)));
            }
        }  // for b
...
void ggml_gemv_tq2_0_8x8_q8_K(int                        n,
    ...
    for (int x = 0; x < nc / ncols_interleaved; x++) {
        ...
        for (int b = 0; b < nb; b++) {
            // acc[cp] lanes: [col 2cp lo, col 2cp hi, col 2cp+1 lo, col 2cp+1 hi]
            int32x4_t acc[col_pairs];
            for (int cp = 0; cp < col_pairs; cp++) {
                acc[cp] = vdupq_n_s32(0);
            }

            for (int h = 0; h < 2; h++) {
                const int8_t * q8_base = q8_ptr[b].qs + h * 128;
                for (int k = 0; k < 4; k++) {
                    // 8 q8 elements per shift, duplicated to both halves to match the column pair
                    const int8x16_t q8_0 = vreinterpretq_s8_s64(vld1q_dup_s64((const int64_t *) (q8_base + 0 * 32 + k * 8)));
                    const int8x16_t q8_1 = vreinterpretq_s8_s64(vld1q_dup_s64((const int64_t *) (q8_base + 1 * 32 + k * 8)));
                    const int8x16_t q8_2 = vreinterpretq_s8_s64(vld1q_dup_s64((const int64_t *) (q8_base + 2 * 32 + k * 8)));
                    const int8x16_t q8_3 = vreinterpretq_s8_s64(vld1q_dup_s64((const int64_t *) (q8_base + 3 * 32 + k * 8)));

                    // chunk (h*4 + k) = 64 bytes: 8 columns x source bytes h*32 + k*8 .. +7
                    const uint8_t * tq_base = tq_ptr[b].qs + (h * 4 + k) * 64;
                    for (int cp = 0; cp < col_pairs; cp++) {
                        const uint8x16_t w = vld1q_u8(tq_base + 16 * cp);
                        acc[cp] = vdotq_s32(acc[cp], tq2_0_codes_0(w, m3), q8_0);
                        acc[cp] = vdotq_s32(acc[cp], tq2_0_codes_2(w, m3), q8_1);
                        acc[cp] = vdotq_s32(acc[cp], tq2_0_codes_4(w, m3), q8_2);
                        acc[cp] = vdotq_s32(acc[cp], tq2_0_codes_6(w), q8_3);
                    }
                }
            }
            ...
            const float32x4_t q8_d = vdupq_n_f32(q8_ptr[b].d);
            for (int c = 0; c < 2; c++) {
                const int32x4_t   sumi  = vpaddq_s32(acc[2 * c], acc[2 * c + 1]);  // cols 4c .. 4c+3
                const float32x4_t tq_d  = vcvt_f32_f16(vld1_f16((const __fp16 *) tq_ptr[b].d + 4 * c));
                const float32x4_t scale = vmulq_f32(tq_d, q8_d);
                acc_f32[c] = vfmaq_f32(acc_f32[c], scale, vcvtq_f32_s32(vsubq_s32(sumi, bsum)));
            }
        }  // for b
...
void ggml_gemm_tq2_0_8x4_q8_K(int                        n,
                              float * GGML_RESTRICT      s,
                              size_t                     bs,
                              const void * GGML_RESTRICT vx,
                              const void * GGML_RESTRICT vy,
    ...
    for (int y = 0; y < nr / q8_k_blocklen; y++) {
        ...
        for (int x = 0; x < nc / ncols_interleaved; x++) {
            ...
            // 4x8 tile: acc_f32[m][c] = token m, columns 4c .. 4c+3
            float32x4_t acc_f32[4][2];
            for (int m = 0; m < 4; m++) {
                acc_f32[m][0] = vdupq_n_f32(0);
                acc_f32[m][1] = vdupq_n_f32(0);
            }

            for (int b = 0; b < nb; b++) {
                int32x4_t acc[4][2];
                for (int m = 0; m < 4; m++) {
                    acc[m][0] = vdupq_n_s32(0);
                    acc[m][1] = vdupq_n_s32(0);
                }

                for (int h = 0; h < 2; h++) {
                    for (int k = 0; k < 8; k++) {
                        // chunk (h*8 + k) = 32 bytes: 8 columns x source bytes h*32 + k*4 .. +3
                        const uint8_t *  tq_base = tq_ptr[b].qs + (h * 8 + k) * 32;
                        const uint8x16_t w0      = vld1q_u8(tq_base);
                        const uint8x16_t w1      = vld1q_u8(tq_base + 16);

                        // q8_Kx4 group g = 16 bytes: 4 tokens x elements 4g .. 4g+3
                        const int8_t * q8_base = q8_ptr[b].qs + (h * 32 + k) * 16;
                        const int8x16_t q8_0   = vld1q_s8(q8_base + 0 * 8 * 16);
                        const int8x16_t q8_1   = vld1q_s8(q8_base + 1 * 8 * 16);
                        const int8x16_t q8_2   = vld1q_s8(q8_base + 2 * 8 * 16);
                        const int8x16_t q8_3   = vld1q_s8(q8_base + 3 * 8 * 16);

                        const int8x16_t c0_0 = tq2_0_codes_0(w0, m3);
                        const int8x16_t c0_1 = tq2_0_codes_0(w1, m3);
                        acc[0][0] = vdotq_laneq_s32(acc[0][0], c0_0, q8_0, 0);
                        acc[1][0] = vdotq_laneq_s32(acc[1][0], c0_0, q8_0, 1);
                        acc[2][0] = vdotq_laneq_s32(acc[2][0], c0_0, q8_0, 2);
                        acc[3][0] = vdotq_laneq_s32(acc[3][0], c0_0, q8_0, 3);
                        acc[0][1] = vdotq_laneq_s32(acc[0][1], c0_1, q8_0, 0);
                        acc[1][1] = vdotq_laneq_s32(acc[1][1], c0_1, q8_0, 1);
                        acc[2][1] = vdotq_laneq_s32(acc[2][1], c0_1, q8_0, 2);
                        acc[3][1] = vdotq_laneq_s32(acc[3][1], c0_1, q8_0, 3);

                        const int8x16_t c2_0 = tq2_0_codes_2(w0, m3);
                        const int8x16_t c2_1 = tq2_0_codes_2(w1, m3);
                        acc[0][0] = vdotq_laneq_s32(acc[0][0], c2_0, q8_1, 0);
                        acc[1][0] = vdotq_laneq_s32(acc[1][0], c2_0, q8_1, 1);
                        acc[2][0] = vdotq_laneq_s32(acc[2][0], c2_0, q8_1, 2);
                        acc[3][0] = vdotq_laneq_s32(acc[3][0], c2_0, q8_1, 3);
                        acc[0][1] = vdotq_laneq_s32(acc[0][1], c2_1, q8_1, 0);
                        acc[1][1] = vdotq_laneq_s32(acc[1][1], c2_1, q8_1, 1);
                        acc[2][1] = vdotq_laneq_s32(acc[2][1], c2_1, q8_1, 2);
                        acc[3][1] = vdotq_laneq_s32(acc[3][1], c2_1, q8_1, 3);

                        const int8x16_t c4_0 = tq2_0_codes_4(w0, m3);
                        const int8x16_t c4_1 = tq2_0_codes_4(w1, m3);
                        acc[0][0] = vdotq_laneq_s32(acc[0][0], c4_0, q8_2, 0);
                        acc[1][0] = vdotq_laneq_s32(acc[1][0], c4_0, q8_2, 1);
                        acc[2][0] = vdotq_laneq_s32(acc[2][0], c4_0, q8_2, 2);
                        acc[3][0] = vdotq_laneq_s32(acc[3][0], c4_0, q8_2, 3);
                        acc[0][1] = vdotq_laneq_s32(acc[0][1], c4_1, q8_2, 0);
                        acc[1][1] = vdotq_laneq_s32(acc[1][1], c4_1, q8_2, 1);
                        acc[2][1] = vdotq_laneq_s32(acc[2][1], c4_1, q8_2, 2);
                        acc[3][1] = vdotq_laneq_s32(acc[3][1], c4_1, q8_2, 3);

                        const int8x16_t c6_0 = tq2_0_codes_6(w0);
                        const int8x16_t c6_1 = tq2_0_codes_6(w1);
                        acc[0][0] = vdotq_laneq_s32(acc[0][0], c6_0, q8_3, 0);
                        acc[1][0] = vdotq_laneq_s32(acc[1][0], c6_0, q8_3, 1);
                        acc[2][0] = vdotq_laneq_s32(acc[2][0], c6_0, q8_3, 2);
                        acc[3][0] = vdotq_laneq_s32(acc[3][0], c6_0, q8_3, 3);
                        acc[0][1] = vdotq_laneq_s32(acc[0][1], c6_1, q8_3, 0);
                        acc[1][1] = vdotq_laneq_s32(acc[1][1], c6_1, q8_3, 1);
                        acc[2][1] = vdotq_laneq_s32(acc[2][1], c6_1, q8_3, 2);
                        acc[3][1] = vdotq_laneq_s32(acc[3][1], c6_1, q8_3, 3);
                    }
                }

                int32_t bsum[4];
                vst1q_s32(bsum, tq2_0_q8_Kx4_bsum(&q8_ptr[b]));
                ...
                for (int m = 0; m < 4; m++) {
                    ...
                    acc_f32[m][0] = vfmaq_f32(acc_f32[m][0], vmulq_f32(tq_d_0, q8_d), vcvtq_f32_s32(vsubq_s32(acc[m][0], bsum_m)));
                    acc_f32[m][1] = vfmaq_f32(acc_f32[m][1], vmulq_f32(tq_d_1, q8_d), vcvtq_f32_s32(vsubq_s32(acc[m][1], bsum_m)));
                }
            }  // for b

            for (int m = 0; m < 4; m++) {
                float * dst = s + (y * q8_k_blocklen + m) * bs + x * ncols_interleaved;
                vst1q_f32(dst, acc_f32[m][0]);
                vst1q_f32(dst + 4, acc_f32[m][1]);
            }
        }  // for x
    }  // for y
    return;
...
void ggml_gemm_tq2_0_8x8_q8_K(int                        n,
                              float * GGML_RESTRICT      s,
                              size_t                     bs,
                              const void * GGML_RESTRICT vx,
                              const void * GGML_RESTRICT vy,
    ...
    for (int y = 0; y < nr / q8_k_blocklen; y++) {
        ...
        for (int x = 0; x < nc / ncols_interleaved; x++) {
            ...
            // 4x8 tile: acc_f32[2*m + c] = token m, columns 4c .. 4c+3
            float32x4_t acc_f32[8];
            for (int i = 0; i < 8; i++) {
                acc_f32[i] = vdupq_n_f32(0);
            }
            ...
            for (int b = 0; b < nb; b++) {
                // acc[cp] = 2x2 tile of column pair cp x tokens 01, acc[cp + 4] = same x tokens 23
                int32x4_t acc[8];
                for (int i = 0; i < 8; i++) {
                    acc[i] = vdupq_n_s32(0);
                }
                ...
                for (int h = 0; h < 2; h++) {
                    for (int k = 0; k < 4; k++) {
                        ...
                        const int8x16_t q8_23_0 = vld1q_s8(q8_base + 0 * 128 + 16);
                        const int8x16_t c0_0    = tq2_0_codes_0(w0, m3);
                        const int8x16_t c0_1    = tq2_0_codes_0(w1, m3);
                        ...
                        acc[4] = vmmlaq_s32(acc[4], c0_0, q8_23_0);
                        acc[5] = vmmlaq_s32(acc[5], c0_1, q8_23_0);
                        acc[6] = vmmlaq_s32(acc[6], c0_2, q8_23_0);
                        acc[7] = vmmlaq_s32(acc[7], c0_3, q8_23_0);

                        const int8x16_t q8_01_1 = vld1q_s8(q8_base + 1 * 128);
                        const int8x16_t q8_23_1 = vld1q_s8(q8_base + 1 * 128 + 16);
                        const int8x16_t c2_0    = tq2_0_codes_2(w0, m3);
                        const int8x16_t c2_1    = tq2_0_codes_2(w1, m3);
                        ...
                        acc[4] = vmmlaq_s32(acc[4], c2_0, q8_23_1);
                        acc[5] = vmmlaq_s32(acc[5], c2_1, q8_23_1);
                        acc[6] = vmmlaq_s32(acc[6], c2_2, q8_23_1);
                        acc[7] = vmmlaq_s32(acc[7], c2_3, q8_23_1);

                        const int8x16_t q8_01_2 = vld1q_s8(q8_base + 2 * 128);
                        const int8x16_t q8_23_2 = vld1q_s8(q8_base + 2 * 128 + 16);
                        const int8x16_t c4_0    = tq2_0_codes_4(w0, m3);
                        const int8x16_t c4_1    = tq2_0_codes_4(w1, m3);
                        ...
                        acc[4] = vmmlaq_s32(acc[4], c4_0, q8_23_2);
                        acc[5] = vmmlaq_s32(acc[5], c4_1, q8_23_2);
                        acc[6] = vmmlaq_s32(acc[6], c4_2, q8_23_2);
                        acc[7] = vmmlaq_s32(acc[7], c4_3, q8_23_2);

                        const int8x16_t q8_01_3 = vld1q_s8(q8_base + 3 * 128);
                        const int8x16_t q8_23_3 = vld1q_s8(q8_base + 3 * 128 + 16);
                        const int8x16_t c6_0    = tq2_0_codes_6(w0);
                        const int8x16_t c6_1    = tq2_0_codes_6(w1);
                        ...
                        acc[4] = vmmlaq_s32(acc[4], c6_0, q8_23_3);
                        acc[5] = vmmlaq_s32(acc[5], c6_1, q8_23_3);
                        acc[6] = vmmlaq_s32(acc[6], c6_2, q8_23_3);
                        acc[7] = vmmlaq_s32(acc[7], c6_3, q8_23_3);
                    }
                }

                // Reorder the 2x2 MMLA tiles into per-token rows of 4 columns (same shuffle as ggml_gemm_q5_K_8x8_q8_K)
                for (int i = 0; i < 8; i++) {
                    int32x2x2_t aux = vzip_s32(vget_low_s32(acc[i]), vget_high_s32(acc[i]));
                    acc[i]          = vcombine_s32(aux.val[0], aux.val[1]);
                }
                const int32x4_t sumi[8] = {
                    vcombine_s32(vget_low_s32(acc[0]), vget_low_s32(acc[1])),
                    vcombine_s32(vget_low_s32(acc[2]), vget_low_s32(acc[3])),
                    vcombine_s32(vget_high_s32(acc[0]), vget_high_s32(acc[1])),
                    vcombine_s32(vget_high_s32(acc[2]), vget_high_s32(acc[3])),
